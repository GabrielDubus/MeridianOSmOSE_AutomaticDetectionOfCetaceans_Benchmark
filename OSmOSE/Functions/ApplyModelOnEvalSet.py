...
def LoadModelHyperParameters(dataset_ID, Task_ID, BM_Name, LengthFile, Fs, Version_name):
    ...
    model_list = ['resnet18', 'resnet50', 'resnet101', 'vgg11', 'vgg11_bn', 'vgg13', 'vgg13_bn', 'vgg19', 'vgg19_bn', 'alexnet', 'squeezenet', 'densenet', 'inception']
    if ModelName in model_list:
        use_pretrained = Parameters['use_pretrained']
    else: use_pretrained = False
...
def ApplyModelOnEvalSet_main(dataset_ID, Task_ID, BM_Name, LengthFile, Fs, Version_name, param_spectro, LabelsList, SplitName):
    ...
    _,_,_,_, ModelName, _,_,_, = LoadModelHyperParameters(dataset_ID, Task_ID, BM_Name, LengthFile, Fs, Version_name)
    
    model_list = ['resnet18', 'resnet50', 'resnet101', 'vgg11', 'vgg11_bn', 'vgg13', 'vgg13_bn', 'vgg19', 'vgg19_bn', 'alexnet', 'squeezenet', 'densenet', 'inception']
    if ModelName in model_list: 
        dataset = ClassDataset(raw_data_path,CSV_annotations_path, param_spectro = param_spectro, transform=transform)
    if ModelName == 'CNN3_FC1' or ModelName == 'CNN3_FC3' :
        dataset = ClassDataset(raw_data_path,CSV_annotations_path, param_spectro = param_spectro, transform=transform_simple)
