...
from Network_Functions import CNN3_FC3, CNN3_FC1, transform_simple, initialize_model, transform, bce_loss
...
def train(device, model_ft, optimizer, num_epochs, train_loader, test_loader, weight, model_path, Version_name, ModelName):
        ...
        if not os.path.exists(model_path + os.sep + Version_name + os.sep + 'model_state' + os.sep + 'sub_state'):
        ...
        for epoch in range(num_epochs):
            ...
            #%% Save Model 
            #save model
            torch.save(model_ft.state_dict(),model_path + os.sep + Version_name + os.sep + 'model_state' + os.sep + 'sub_state' + os.sep + Version_name + '_epoch'+ str(int(epoch))+ '_model.pt')
            # save model as script
            if ModelName != 'CNN3_RNN1':
                model_ft_scripted = torch.jit.script(model_ft) # Export to TorchScript
                model_ft_scripted.save(model_path + os.sep + Version_name + os.sep + 'model_state' + os.sep + 'sub_state' + os.sep + Version_name + '_epoch'+ str(int(epoch)) + '_Scripted_model.pt') # Save
            
        return loss_tab_train, loss_tab_test

def TrainNetwork_main(dataset_ID, Task_ID, BM_Name, LengthFile, Fs, SplitName, Version_name, ModelName, use_pretrained, TrainSetRatio, batch_size, learning_rate, num_epochs, param_spectro):
    ...
    '''
        INPUTS :
            ...
            - param_spectro : parameters for spectrograms computation
        '''
    ...
    path_wav_files = path_osmose_dataset + dataset_ID + os.sep + 'raw' + os.sep + 'audio' + os.sep + folderName_audioFiles 
    ...
    model_list = ['resnet18', 'resnet50', 'resnet101', 'vgg11', 'vgg11_bn', 'vgg13', 'vgg13_bn', 'vgg19', 'vgg19_bn', 'alexnet']
...
#%% DEF TEST AND TRAIN SET
    ...
    '''
    Nb : For the moment, the split is simply done by cutting in one spot and mix, new possibilities will be added
    '''
    
    raw_data_path = path_wav_files
    ...
    dataset = ClassDataset(raw_data_path,CSV_annotations_path, param_spectro = param_spectro, transform=trans)
        
   
    array_ind = np.arange(0,NbFile,1)
    
    array_ind = np.random.choice(array_ind, NbFileUsed)
    
    # Created using indices from 0 to train_size.
    train_set = torch.utils.data.Subset(dataset, array_ind[:int(TrainSetRatio*NbFileUsed)])
    ...
    test_set = torch.utils.data.Subset(dataset, array_ind[int(TrainSetRatio*NbFileUsed):])
    ...
    if ModelName in model_list:
        ...
        optimizer = torch.optim.SGD(model_ft.parameters(), lr=learning_rate, momentum=0.9)
